...
implicit none
    ...
    write(p%fil%c,*)p%glb%time,100.0d0*(p%glb%icsum-p%glb%csum)/p%glb%icsum
    write(p%fil%energy,*)p%glb%time,p%glb%e_kine,p%glb%e_poten,p%glb%e_diss,p%glb%e_sed

end subroutine

subroutine print_c_info()
use all
implicit none
    write(*,'("Loss of C (%):",ES15.4)')100.0d0*(p%glb%icsum-p%glb%csum)/p%glb%icsum
    write(*,*)''
    write(*,'("Energy Loss(%):",ES15.4)')100.0d0*(p%glb%e_t0-p%glb%e_t)/p%glb%e_t0
end subroutine
