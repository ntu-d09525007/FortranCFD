...
do id = 0, p%glb%threads-1
    ...
    if( p%of(id)%loc%idz==0 )then
        ...
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
        do j = p%of(id)%loc%js, p%of(id)%loc%je
            src = p%glb%us_c*(p%of(id)%loc%c%now(i,j,2)-p%of(id)%loc%c%now(i,j,1))/p%glb%dz*p%glb%dt
            p%of(id)%loc%c%now(i,j,1) = p%of(id)%loc%c%now(i,j,1) + src
        enddo
        enddo
...
integer :: id,i,j,k
    ...
    do id = 0, p%glb%threads-1
        ...
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
            ...
            p%of(id)%loc%nvel%z%old(i,j,k) = 0.5d0*( p%of(id)%loc%vel%z%old(i,j,k-1)+p%of(id)%loc%vel%z%old(i,j,k) ) - p%glb%us_c
...
integer :: i,j,k
    ...
    do j = q%loc%js, q%loc%je
        call q%loc%ccdsolvers%x%solve("uccd",q%loc%c%now(:,j,k),q%loc%tdata%x%s1(:,j,k),q%loc%tdata%x%s2(:,j,k),q%loc%nvel%x%old(:,j,k))
    end do 
    end do

    do k = q%loc%ks, q%loc%ke
    do i = q%loc%is, q%loc%ie
        call q%loc%ccdsolvers%y%solve("uccd",q%loc%c%now(i,:,k),q%loc%tdata%y%s1(i,:,k),q%loc%tdata%y%s2(i,:,k),q%loc%nvel%y%old(i,:,k))
    end do 
    end do

    do j = q%loc%js, q%loc%je
    do i = q%loc%is, q%loc%ie
        call q%loc%ccdsolvers%z%solve("uccd",q%loc%c%now(i,j,:),q%loc%tdata%z%s1(i,j,:),q%loc%tdata%z%s2(i,j,:),q%loc%nvel%z%old(i,j,:))
    end do 
    end do
