...
logical :: btn
	...
	do id = 0, p%glb%threads-1
		...
		do j = p%of(id)%loc%js-p%glb%ghc, p%of(id)%loc%je+p%glb%ghc
			...
			call p%of(id)%loc%ccd%x%SOLVE_SRKCCD(p%of(id)%loc%nvel%x%old(:,j,k),&
		...
		do i = p%of(id)%loc%is-p%glb%ghc, p%of(id)%loc%ie+p%glb%ghc
			...
			call p%of(id)%loc%ccd%y%SOLVE_SRKCCD(p%of(id)%loc%nvel%y%old(i,:,k),&
		...
		do i = p%of(id)%loc%is-p%glb%ghc, p%of(id)%loc%ie+p%glb%ghc
			...
			call p%of(id)%loc%ccd%z%SOLVE_SRKCCD(p%of(id)%loc%nvel%z%old(i,j,:),&
