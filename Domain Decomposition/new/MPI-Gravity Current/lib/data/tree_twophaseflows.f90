...
real(8) :: heavy, rhod
    ...
    rhod = (( p%glb%re * p%glb%mu_1 ) / ( p%glb%rho_1 * p%glb%L ))**2.0d0 / (p%glb%g*p%glb%L)
    ...
    do id = 0, p%glb%threads-1
        ...
        do i = p%of(id)%loc%is-p%glb%ghc, p%of(id)%loc%ie+p%glb%ghc
            ...
            p%of(id)%loc%mu%now(i,j,k)  = 1.0d0 + rhod * p%of(id)%loc%c%now(i,j,k)
...
real(8),dimension(p%glb%mpisize) :: buffer
    ...
    call mpi_allgather(mass, 1, mpi_real8, &
                       buffer, 1, mpi_real8, &
                       mpi_comm_world,ierr)
    p%glb%mass = sum(buffer)

    call mpi_allgather(vol, 1, mpi_real8, &
                       buffer, 1, mpi_real8, &
                       mpi_comm_world,ierr)
    p%glb%vol = sum(buffer)
    ...
    call mpi_allgather(mass, 1, mpi_real8, &
                       buffer, 1, mpi_real8, &
                       mpi_comm_world,ierr)
    p%glb%massv = sum(buffer)

    call mpi_allgather(vol, 1, mpi_real8, &
                       buffer, 1, mpi_real8, &
                       mpi_comm_world,ierr)
    p%glb%volv = sum(buffer)
