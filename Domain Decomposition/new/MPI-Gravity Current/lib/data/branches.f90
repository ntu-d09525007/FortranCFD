...
integer :: grid_x, grid_y, grid_z, grid_A, threads, num_domains ! numbers of CPUs 
integer :: mpirank, mpisize, grids(3,2), mpidim(2)
integer,allocatable,dimension(:) :: mpithreads, mpigrid, mpis, mpie
integer,allocatable :: tid(:,:,:), xid(:), yid(:), zid(:)
...
subroutine job_init(p,id,gx,gy,gz,ids)
implicit none
class(job) :: p
integer :: gx,gy,gz
integer :: id, IS, IE, JS, JE, KS, KE, ids
integer :: nx, ny, nz, level

if( p%glb%split==3 )then

    P%LOC%ID = ID+(ids-1)*(gx*gy)
    p%loc%idz = P%LOC%ID / (gx*gy)
    p%loc%idx = (P%LOC%ID - p%loc%idz*gx*gy) / gy
    p%loc%idy = (P%LOC%ID - p%loc%idz*gx*gy - p%loc%idx*gy)

else if( p%glb%split==2 )then

    P%LOC%ID = ID+(ids-1)*(gx*gz)
    p%loc%idy = P%LOC%ID / (gx*gz)
    p%loc%idz = (P%LOC%ID - p%loc%idy*gx*gz) / gx
    p%loc%idx = (P%LOC%ID - p%loc%idy*gx*gz - p%loc%idz*gx)

else 

    P%LOC%ID = ID+(ids-1)*(gz*gy)
    p%loc%idx = P%LOC%ID / (gz*gy)
    p%loc%idy = (P%LOC%ID - p%loc%idx*gz*gy) / gz
    p%loc%idz = (P%LOC%ID - p%loc%idx*gz*gy - p%loc%idy*gz)
...
call p%loc%c%alloc(IS,IE,JS,JE,KS,KE)
...
do level = 1, p%glb%level
    ...
    p%loc%mg(level)%grids = p%glb%grids
