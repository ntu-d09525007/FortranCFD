...
real(8) :: sump, err, w, pcal, tol, err2
...
do
    ...
    err=0.0d0
    err2=0.0d0    
    !$omp parallel do private(i,j,k), reduction(+:err), reduction(max:err2)
    do id = 0, p%glb%threads-1
        ...
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
            ...
            err2 = max( err2,abs(p%of(id)%loc%p%now(i,j,k)-p%of(id)%loc%p%tmp(i,j,k)) )
