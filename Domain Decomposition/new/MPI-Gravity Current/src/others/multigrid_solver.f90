...
integer :: nx,ny,nz,ghc,num
integer :: ix,iy,iz,iid
integer :: ierr, istat
integer :: mdim(2)
real(8),dimension(:,:,:,:), allocatable :: mpi_sendp, mpi_sendm
real(8),dimension(:,:,:,:), allocatable :: mpi_recvp, mpi_recvm
real(8),dimension(:). allocatable :: mpi_buffer, mpi_buffer2
...
if( p%mpisize>1 )then
    ...
    if( p%split == 1)then
        ...
        allocate(mpi_sendp(p%glb%grids(2,1):p%glb%grids(2,2),p%glb%grids(3,1):p%glb%grids(3,2),ny,nz),&
                 mpi_sendm(p%glb%grids(2,1):p%glb%grids(2,2),p%glb%grids(3,1):p%glb%grids(3,2),ny,nz),&
                 mpi_recvp(p%glb%grids(2,1):p%glb%grids(2,2),p%glb%grids(3,1):p%glb%grids(3,2),ny,nz),&
                 mpi_recvm(p%glb%grids(2,1):p%glb%grids(2,2),p%glb%grids(3,1):p%glb%grids(3,2),ny,nz))

        num = (1-p%glb%grids(2,1)+p%glb%grids(2,2))*(1-p%glb%grids(3,1)+p%glb%grids(3,2))

        mdim(1) = 1-p%glb%grids(2,1)+p%glb%grids(2,2)
        mdim(2) = 1-p%glb%grids(3,1)+p%glb%grids(3,2)

        allocate(mpi_buffer(num*ny*nz),mpi_buffer2(num*ny*nz))

        ix=p%glb%grids(1,1)
        do iy = p%glb%grids(2,1), p%glb%grids(2,2)
        do iz = p%glb%grids(3,1), p%glb%grids(3,2)
            id=p%glb%tid(ix,iy,iz)
            do j = 1, ny
            do k = 1, nz
                mpi_sendm(iy,iz,j,k) = p%of(id)%loc%mg(level)%sol(1,j,k)
            enddo
            enddo
        enddo
        enddo
        ...
        do iz = p%glb%grids(3,1), p%glb%grids(3,2)
            ...
            do j = 1, ny
            do k = 1, nz
                mpi_sendp(iy,iz,j,k) = p%of(id)%loc%mg(level)%sol(nx,j,k)
            enddo
            enddo
        enddo
        enddo
        ...
        allocate(mpi_sendp(p%glb%grids(1,1):p%glb%grids(1,2),p%glb%grids(3,1):p%glb%grids(3,2),nx,nz),&
                 mpi_sendm(p%glb%grids(1,1):p%glb%grids(1,2),p%glb%grids(3,1):p%glb%grids(3,2),nx,nz),&
                 mpi_recvp(p%glb%grids(1,1):p%glb%grids(1,2),p%glb%grids(3,1):p%glb%grids(3,2),nx,nz),&
                 mpi_recvm(p%glb%grids(1,1):p%glb%grids(1,2),p%glb%grids(3,1):p%glb%grids(3,2),nx,nz))

        num = (1-p%glb%grids(1,1)+p%glb%grids(1,2))*(1-p%glb%grids(3,1)+p%glb%grids(3,2))

        mdim(1) = 1-p%glb%grids(1,1)+p%glb%grids(1,2)
        mdim(2) = 1-p%glb%grids(3,1)+p%glb%grids(3,2)

        allocate(mpi_buffer(num*nx*nz),mpi_buffer2(num*nx*nz))

        iy=p%glb%grids(2,1)
        do ix = p%glb%grids(1,1), p%glb%grids(1,2)
        do iz = p%glb%grids(3,1), p%glb%grids(3,2)
            id=p%glb%tid(ix,iy,iz)
            do i = 1, nx
            do k = 1, nz
                mpi_sendm(ix,iz,i,k) = p%of(id)%loc%mg(level)%sol(i,1,k)
            enddo
            enddo
        enddo
        enddo
        ...
        do iz = p%glb%grids(3,1), p%glb%grids(3,2)
            ...
            do i = 1, nx
            do k = 1, nz
                mpi_sendp(ix,iz,i,k) = p%of(id)%loc%mg(level)%sol(i,ny,k)
            enddo
            enddo
        enddo
        enddo
        ...
        allocate(mpi_sendp(p%glb%grids(1,1):p%glb%grids(1,2),p%glb%grids(2,1):p%glb%grids(2,2),nx,ny),&
                 mpi_sendm(p%glb%grids(1,1):p%glb%grids(1,2),p%glb%grids(2,1):p%glb%grids(2,2),nx,ny),&
                 mpi_recvp(p%glb%grids(1,1):p%glb%grids(1,2),p%glb%grids(2,1):p%glb%grids(2,2),nx,ny),&
                 mpi_recvm(p%glb%grids(1,1):p%glb%grids(1,2),p%glb%grids(2,1):p%glb%grids(2,2),nx,ny))

        num = (1-p%glb%grids(1,1)+p%glb%grids(1,2))*(1-p%glb%grids(2,1)+p%glb%grids(2,2))

        mdim(1) = 1-p%glb%grids(1,1)+p%glb%grids(1,2)
        mdim(2) = 1-p%glb%grids(2,1)+p%glb%grids(2,2)

        allocate(mpi_buffer(num*ny*nx),mpi_buffer2(num*ny*nx))
        ...
        do iy = p%glb%grids(2,1), p%glb%grids(2,2)
            ...
            do i = 1, nx
            do j = 1, ny
                mpi_sendm(ix,iy,i,j) = p%of(id)%loc%mg(level)%sol(i,j,1)
            enddo
            enddo
        enddo
        enddo
        ...
        do iy = p%glb%grids(2,1), p%glb%grids(2,2)
            ...
            do i = 1, nx
            do j = 1, ny
                mpi_sendp(ix,iy,i,j) = p%of(id)%loc%mg(level)%sol(i,j,nz)
            enddo
            enddo
        enddo
        enddo
    ...
    if( p%glb%mpirank < p%glb%mpisize-1 )then
        mpi_buffer = reshape( mpi_sendp, (/ num*nx*ny /) )
        call mpi_send( mpi_buffer, num*nx*ny, mpi_real8, p%glb%mpirank+1, 100*p%glb%mpirank+1, mpi_comm_world, ierr)
    endif

    if( p%glb%mpirank > 0 )then
        call mpi_recv(mpi_buffer2, num*nx*ny, mpi_real8, p%glb%mpirank-1, 100*(p%glb%mpirank-1)+1, mpi_comm_world, istat, ierr )
        mpi_recvp = reshape( mpi_buffer2 , (/ mdim(1),mdim(2),nx,ny /) )
    endif

    if( p%glb%mpirank > 0 )then
        mpi_buffer = reshape( mpi_sendm, (/ num*nx*ny /) )
        call mpi_send( mpi_buffer, num*nx*ny, mpi_real8, p%glb%mpirank-1, 200*p%glb%mpirank+2, mpi_comm_world, ierr)
    endif

    if( p%glb%mpirank < p%glb%mpisize-1 )then
        call mpi_recv(mpi_buffer2, num*nx*ny, mpi_real8, p%glb%mpirank+1, 200*(p%glb%mpirank+1)+2, mpi_comm_world, istat, ierr )
        mpi_recvm = reshape( mpi_buffer2 , (/ mdim(1),mdim(2),nx,ny /) )
    endif
...
!$omp parallel do private(i,j,k,ix,iy,iz,iid)
do id = 0, p%glb%threads-1  
    
    !==================================================================
    
    if( p%of(id)%loc%idx > p%glb%grids(1,1) )then
        do k = 1, nz
        do j = 1, ny
            ix = p%glb%xid(id); iy = p%glb%yid(id); iz = p%glb%zid(id)
            iid = p%glb%tid(ix-1,iy,iz)
            p%of(id)%loc%mg(level)%sol(0,j,k)=p%of(iid)%loc%mg(level)%sol(nx,j,k)
        enddo
        enddo
    else
        do k = 1, nz
    ...
    if( p%of(id)%loc%idx < p%glb%grids(1,2) )then
        do k = 1, nz
        do j = 1, ny
            ix = p%glb%xid(id); iy = p%glb%yid(id); iz = p%glb%zid(id)
            iid = p%glb%tid(ix+1,iy,iz)
            p%of(id)%loc%mg(level)%sol(nx+1,j,k)=p%of(iid)%loc%mg(level)%sol(1,j,k)
        enddo
        enddo
    else
        do k = 1, nz
    ...
    if( p%of(id)%loc%idy > p%glb%grids(2,1) )then
        do k = 1, nz
        do i = 1, nx
            ix = p%glb%xid(id); iy = p%glb%yid(id); iz = p%glb%zid(id)
            iid = p%glb%tid(ix,iy-1,iz)
            p%of(id)%loc%mg(level)%sol(i,0,k)=p%of(iid)%loc%mg(level)%sol(i,ny,k)
        enddo
        enddo
    else
        do k = 1, nz
    ...
    if( p%of(id)%loc%idy < p%glb%grids(2,2) )then
        do k = 1, nz
        do i = 1, nx
            ix = p%glb%xid(id); iy = p%glb%yid(id); iz = p%glb%zid(id)
            iid = p%glb%tid(ix,iy+1,iz)
            p%of(id)%loc%mg(level)%sol(i,ny+1,k)=p%of(iid)%loc%mg(level)%sol(i,1,k)
        enddo
        enddo
    else
        do k = 1, nz
    ...
    if( p%of(id)%loc%idz > p%glb%grids(3,1) )then
        do j = 1, ny
        do i = 1, nx
            ix = p%glb%xid(id); iy = p%glb%yid(id); iz = p%glb%zid(id)
            iid = p%glb%tid(ix,iy,iz-1)
            p%of(id)%loc%mg(level)%sol(i,j,0)=p%of(iid)%loc%mg(level)%sol(i,j,nz)
        enddo
        enddo
    else
        do j = 1, ny
    ...
    if( p%of(id)%loc%idz < p%glb%grids(3,2) )then
        do j = 1, ny
        do i = 1, nx
            ix = p%glb%xid(id); iy = p%glb%yid(id); iz = p%glb%zid(id)
            iid = p%glb%tid(ix,iy,iz+1)
            p%of(id)%loc%mg(level)%sol(i,j,nz+1)=p%of(iid)%loc%mg(level)%sol(i,j,1)
        enddo
        enddo
    else
        do j = 1, ny
