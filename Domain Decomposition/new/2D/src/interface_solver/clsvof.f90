...
integer(8) :: cpustart, cpuend
    ...
    do id = 0, p%glb%threads-1
        ...
        do i = p%of(id)%loc%is-1, p%of(id)%loc%ie
            ...
            if( p%of(id)%loc%vof%now(ii,j) > 1.0d0-eps .or. p%of(id)%loc%vof%now(ii,j) < eps )then
               ...
               p%of(id)%loc%tdata%x%l1(i,j) = p%of(id)%loc%vof%now(ii,j) * p%of(id)%loc%vel%x%old(i,j) * p%glb%dt
                ...
                p%of(id)%loc%tdata%x%l1(i,j) = 0.5d0*( p%of(id)%loc%vel%x%old(i,j)*p%glb%dt - alpha*p%glb%dx/beta*dlog(a4/a5) )
            
                p%of(id)%loc%tdata%x%l1(i,j) = p%of(id)%loc%tdata%x%l1(i,j)*w + ( 1.0d0 - w ) * p%of(id)%loc%vof%now(ii,j) * p%of(id)%loc%vel%x%old(i,j) * p%glb%dt
        ...
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
            ...
            p%of(id)%loc%vof%now(i,j) = p%of(id)%loc%vof%now(i,j) - ( p%of(id)%loc%tdata%x%l1(i,j) - p%of(id)%loc%tdata%x%l1(i-1,j) ) / p%glb%dx + &
    ...
    do id = 0, p%glb%threads-1
        ...
        do i = p%of(id)%loc%is-1, p%of(id)%loc%ie
            ...
            if( p%of(id)%loc%vof%now(i,ii) > 1.0d0-eps .or. p%of(id)%loc%vof%now(i,ii) < eps )then
               
               p%of(id)%loc%tdata%y%l1(i,j) = p%of(id)%loc%vof%now(i,ii) * p%of(id)%loc%vel%y%old(i,j) * p%glb%dt
              
            else
            
                ib = max(1,ii-1)
                
                if( p%of(id)%loc%vof%now(i,ib) < p%of(id)%loc%vof%now(i,ii+1) )then
                    alpha = 1.0d0
                else
                    alpha = -1.0d0
                endif
                
                a1 = dexp( beta*( 2.0d0 * p%of(id)%loc%vof%now(i,ii) - 1.0d0 ) / alpha )
                a3 = dexp( beta )
                xc = 0.5d0 / beta * dlog( ( a3*a3-a1*a3)/(a1*a3-1.0d0) )
                a4 = dcosh( beta * ( isgn - p%of(id)%loc%vel%y%old(i,j)*p%glb%dt/p%glb%dy - xc ) )
                a5 = dcosh( beta * ( isgn - xc ) )
                
                w  = abs(p%of(id)%loc%normals%x%now(i,ii)) + abs(p%of(id)%loc%normals%y%now(i,ii))
                w  = abs(p%of(id)%loc%normals%y%now(i,ii)) / w
            
                p%of(id)%loc%tdata%y%l1(i,j) = 0.5d0*( p%of(id)%loc%vel%y%old(i,j)*p%glb%dt - alpha*p%glb%dy/beta*dlog(a4/a5) )
            
                p%of(id)%loc%tdata%y%l1(i,j) = p%of(id)%loc%tdata%y%l1(i,j)*w + ( 1.0d0 - w ) * p%of(id)%loc%vof%now(i,ii) * p%of(id)%loc%vel%y%old(i,j) * p%glb%dt
        ...
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
            ...
            p%of(id)%loc%vof%now(i,j) = p%of(id)%loc%vof%now(i,j) - ( p%of(id)%loc%tdata%y%l1(i,j) - p%of(id)%loc%tdata%y%l1(i,j-1) ) / p%glb%dy + &
...
do
    ...
    do id = 0, p%glb%threads-1
        ...
        do j = p%of(id)%loc%js-p%glb%ghc, p%of(id)%loc%je+p%glb%ghc
        do i = p%of(id)%loc%is-p%glb%ghc, p%of(id)%loc%ie+p%glb%ghc
            if( abs(p%of(id)%loc%phi%tmp(i,j)) < p%glb%ls_wid )p%of(id)%loc%phi%tmp(i,j)=p%of(id)%loc%phi%now(i,j)
            p%of(id)%loc%phi%now(i,j) = p%of(id)%loc%phi%tmp(i,j)
        end do
        end do
