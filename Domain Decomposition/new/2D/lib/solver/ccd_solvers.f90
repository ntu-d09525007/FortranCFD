...
integer :: is, ie
real(8) :: dx, dt
type(working_matrix) :: ccd, uccd, srkccd
contains
procedure init => root_init
...
subroutine root_init(p,is,ie,dx,dt)
implicit none
class(ccd_root) :: p
integer :: is,ie
real(8) :: dx,dt

p%is=is; p%ie=ie; p%dx=dx; p%dt=dt

call p%ccd%alloc(p%is,p%ie,p%dx,p%dt)
call p%ccd%init_ccd
call p%ccd%decom(.false.)

call p%uccd%alloc(p%is,p%ie,p%dx,p%dt)
call p%uccd%init_uccd
call p%uccd%decom(.true.)
...
select case (trim(type))
    case ("ccd")
        call p%ccd%asgn_src_ccd(f)
        call p%ccd%solve(.false.)
        do i = p%is, p%ie
            fx(i) = p%ccd%su(i)
            fxx(i) = p%ccd%ssu(i)
        enddo
    case ("uccd")
        call p%uccd%asgn_src_uccd(f)
        call p%uccd%solve(.true.)
        do i = p%is, p%ie
            if( u(i)>0.0d0 )then
                fx(i) = p%uccd%su(i)
            else
            ...
            fxx(i) = 0.5d0*( p%ccd%ssu(i)+p%ccd%ssd(i) )
        enddo
    case ("srkccd")
        call p%srkccd%init_srkccd_c3(u)
        call p%srkccd%init_srkccd
        call p%srkccd%decom(.true.)
        call p%srkccd%asgn_src_srkccd(f)
        call p%srkccd%solve(.true.)
        do i = p%is, p%ie
            if( u(i)>0.0d0 )then
                fx(i) = p%srkccd%su(i)
            else
