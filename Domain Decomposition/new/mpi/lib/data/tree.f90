...
type mpiinfo
integer :: id, szie, split
integer,allocatable,dimension(:) :: threads
end type mpiinfo
...
integer :: max_threads
integer :: i,j,k,id
integer :: buffer, istat, ierr
real(8) :: mag

    if( p%mpi%id==0 )then
        call p%read(path)
    else
        call mpi_recv(buffer,1,mpi_int,p%mpi%id-1,0,mpi_comm_world,istat,ierr)
        call p%read(path)
    endif

    if( p%mpi%id<p%mpi%size-1 )call mpi_send(0,1,mpi_int,p%mpi%id+1,0,mpi_comm_world,ierr)
    ...
    !===================== Assign domains to MPI unit =================!
    if( p%mpi%size>1 )then
        if( p%glb%grid_x >= p%glb%grid_y .and. p%glb%grid_x >= p%glb%grid_z )then
            p%mpi%split = 1
        endif  
    else
