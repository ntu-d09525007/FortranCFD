...
type ptr_family
integer :: threads
type(pointer_parent) :: phi, p, vof, c, s, solid
...
integer :: id
    ...
    call this%s%init(src)
    call this%solid%init(src)
    ...
    do id = 0, this%threads-1
        ...
        this%s%of(id)%dat => src%of(id)%loc%s%now
        this%solid%of(id)%dat => src%of(id)%loc%solid%now
