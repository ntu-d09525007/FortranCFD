...
procedure IBM_vel => manager_IBM_vel
...
character(*) :: path
 ...
 read(526,*)p%glb%us_c
 read(526,*)
 read(526,*)p%glb%h0, p%glb%H, p%glb%slope
 read(526,*)
 read(526,*)p%glb%strat, p%glb%rhoc
...
real(8) :: rhoA, rhoh0, rhoB
    ...
    p%fil%c = 16
    open(unit=p%fil%c,file="./out/"//trim(p%glb%name)//"_CS_loss.plt")
    write(p%fil%c,*)'variables = "T" "Loss of concentration(%)" "Loss of Saline(%)"'
    ...
    rhoh0 = p%glb%strat * (p%glb%rhoc-p%glb%rho_1) + p%glb%rho_1
    rhoA = p%glb%rho_1 + 0.5d0*( rhoh0-p%glb%rho_1 ) 
    rhoB = p%glb%rho_1 + p%glb%H / p%glb%h0 * ( rhoh0-p%glb%rho_1 ) 
    
    p%glb%alpha_c = (p%glb%rhoc-p%glb%rho_1)/(p%glb%rhoc-rhoA) * p%glb%H / p%glb%h0
    p%glb%alpha_s = (rhoB - p%glb%rho_1)/(p%glb%rhoc-rhoA) * p%glb%H / p%glb%h0
...
real(8) :: dv, csum, ssum
    ...
    csum=0.0d0
    ssum=0.0d0
    !$omp parallel do private(i,j,k), reduction(+:csum,ssum)    
    do id = 0, p%glb%threads-1
        ...
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
            ...
            ssum = ssum + p%of(id)%loc%s%now(i,j,k)*dv
...
do id = 0, p%glb%threads-1
    ...
    do i = p%of(id)%loc%is, p%of(id)%loc%ie
        ...
        p%of(id)%loc%vel%x%now(i,j,k) = (1.0d0-solid)*p%of(id)%loc%vel%x%now(i,j,k)
        
        solid = 0.5d0*( p%of(id)%loc%solid%now(i,j,k)+p%of(id)%loc%solid%now(i,j+1,k) )
        p%of(id)%loc%vel%y%now(i,j,k) = (1.0d0-solid)*p%of(id)%loc%vel%y%now(i,j,k)
        
        solid = 0.5d0*( p%of(id)%loc%solid%now(i,j,k)+p%of(id)%loc%solid%now(i,j,k+1) )
        p%of(id)%loc%vel%z%now(i,j,k) = (1.0d0-solid)*p%of(id)%loc%vel%z%now(i,j,k)
        
        !===================================================
        
        solid = p%of(id)%loc%solid%now(i,j,k)
        
        p%of(id)%loc%nvel%x%now(i,j,k) = (1.0d0-solid)*p%of(id)%loc%nvel%x%now(i,j,k)
        p%of(id)%loc%nvel%y%now(i,j,k) = (1.0d0-solid)*p%of(id)%loc%nvel%y%now(i,j,k)
        p%of(id)%loc%nvel%z%now(i,j,k) = (1.0d0-solid)*p%of(id)%loc%nvel%z%now(i,j,k)
