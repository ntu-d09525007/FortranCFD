...
implicit none
    ...
    write(p%fil%c,*)p%glb%time,100.0d0*(p%glb%icsum-p%glb%csum)/p%glb%icsum,100.0d0*(p%glb%issum-p%glb%ssum)/p%glb%issum
...
implicit none
	...
	write(*,'("Loss of S (%):",ES15.4)')100.0d0*(p%glb%issum-p%glb%ssum)/p%glb%issum
    write(*,*)''
end subroutine
