...
real(8) :: cnew,cold,snew,sold

    !$omp parallel do private(i,j,k,rho,mu,delta,xx,yy,zz), &
    !$omp& private(ux,uy,uz,vx,wx,phix,phiy,phiz,dif_x,dif_y,dif_z,curv,cnew,cold,snew,sold)
    do id = 0, p%glb%threads-1
        ...
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
			...
			snew = 0.5d0*( p%of(id)%loc%s%now(i,j,k)+p%of(id)%loc%s%now(i+1,j,k) )
            sold = 0.5d0*( p%of(id)%loc%s%old(i,j,k)+p%of(id)%loc%s%old(i+1,j,k) )
            
            p%of(id)%loc%velsrc%x%tmp(i,j,k) = 0.5d0*(dif_x + dif_y + dif_z) !+ 0.5d0*(p%glb%alpha_c*(cnew+cold)+p%glb%alpha_s*(snew+sold))*p%glb%gx 
...
real(8) :: cnew,cold,snew,sold
    ...
    !$omp parallel do private(i,j,k,rho,mu,delta,xx,yy,zz), &
    !$omp& private(vx,vy,vz,wy,uy,phix,phiy,phiz,dif_x,dif_y,dif_z,curv,cnew,cold,snew,sold)
    do id = 0, p%glb%threads-1
        ...
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
			...
			snew = 0.5d0*( p%of(id)%loc%s%now(i,j,k)+p%of(id)%loc%s%now(i,j+1,k) )
            sold = 0.5d0*( p%of(id)%loc%s%old(i,j,k)+p%of(id)%loc%s%old(i,j+1,k) )
			
            p%of(id)%loc%velsrc%y%tmp(i,j,k) = 0.5d0*(dif_x + dif_y + dif_z) !+ 0.5d0*(p%glb%alpha_c*(cnew+cold)+p%glb%alpha_s*(snew+sold))*p%glb%gy 
...
real(8) :: cnew,cold,snew,sold
    ...
    !$omp parallel do private(i,j,k,rho,mu,delta,xx,yy,zz), &
    !$omp& private(wx,wy,wz,uz,vz,phix,phiy,phiz,dif_x,dif_y,dif_z,curv,cnew,cold,snew,sold)
    do id = 0, p%glb%threads-1
        ...
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
			...
			snew = 0.5d0*( p%of(id)%loc%s%now(i,j,k)+p%of(id)%loc%s%now(i,j,k+1) )
            sold = 0.5d0*( p%of(id)%loc%s%old(i,j,k)+p%of(id)%loc%s%old(i,j,k+1) )
			
            p%of(id)%loc%velsrc%z%tmp(i,j,k) = 0.5d0*(dif_x + dif_y + dif_z) - 0.5d0*(p%glb%alpha_c*(cnew+cold)+p%glb%alpha_s*(snew+sold))!*p%glb%gz 
