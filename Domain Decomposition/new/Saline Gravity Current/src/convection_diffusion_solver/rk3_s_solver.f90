...
real(8) :: src
    ...
    do id = 0, p%glb%threads-1
        ...
        call s_rk3_source(p%of(id),p%of(id)%loc%tdata%x%l1)
        
        do k = p%of(id)%loc%ks, p%of(id)%loc%ke
        do j = p%of(id)%loc%js, p%of(id)%loc%je
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
            src = p%of(id)%loc%tdata%x%l1(i,j,k)
            p%of(id)%loc%s%now(i,j,k) = p%of(id)%loc%s%now(i,j,k) + src * p%glb%dt  
            p%of(id)%loc%s%now(i,j,k) = max(min(1.0d0,p%of(id)%loc%s%now(i,j,k)),0.0d0)         
        end do 
        end do 
        end do
    ...
    do id = 0, p%glb%threads-1
        ...
        call s_rk3_source(p%of(id),p%of(id)%loc%tdata%x%l2)
    ...
    do id = 0, p%glb%threads-1
        ...
        call s_rk3_source(p%of(id),p%of(id)%loc%tdata%x%l3)
...
integer :: i,j,k
    ...
    do j = q%loc%js, q%loc%je
        call q%loc%uccd%x%solve(q%loc%nvel%x%old(:,j,k),q%loc%s%now(:,j,k),q%loc%tdata%x%s1(:,j,k),q%loc%tdata%x%s2(:,j,k))
    end do 
    end do

    do k = q%loc%ks, q%loc%ke
    do i = q%loc%is, q%loc%ie
        call q%loc%uccd%y%solve(q%loc%nvel%y%old(i,:,k),q%loc%s%now(i,:,k),q%loc%tdata%y%s1(i,:,k),q%loc%tdata%y%s2(i,:,k))
    end do 
    end do

    do j = q%loc%js, q%loc%je
    do i = q%loc%is, q%loc%ie
        call q%loc%uccd%z%solve(q%loc%nvel%z%old(i,j,:),q%loc%s%now(i,j,:),q%loc%tdata%z%s1(i,j,:),q%loc%tdata%z%s2(i,j,:))
    end do 
    end do
