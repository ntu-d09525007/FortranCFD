...
do
	...
	!$omp parallel private(id,i,j,k), num_threads(p%glb%threads), reduction(max:error)
...
real(8) :: grad
		...
		!$omp parallel private(id,i,j,k), reduction(max:grad), num_threads(p%glb%threads)
...
real(8) :: upp,upm,ump,umm,vpp,vpm,vmp,vmm,wpp,wpm,wmp,wmm
	...
	!$omp parallel private(id,i,j,k,upp,upm,ump,umm,vpp,vpm,vmp,vmm,wpp,wpm,wmp,wmm), num_threads(p%glb%threads)
...
real(8) :: a,b,dv
	...
	!$omp parallel private(id,i,j,k), num_threads(p%glb%threads)
		...
		do i = p%of(id)%loc%is, p%of(id)%loc%ie
			...
			p%of(id)%loc%tdata%x%s2(i,j,k) = p%of(id)%loc%delta%now(i,j,k)*p%of(id)%loc%sign%tmp(i,j,k)*( p%of(id)%loc%grad%now(i,j,k) - 1.0d0 ) 
			p%of(id)%loc%tdata%x%s3(i,j,k) = p%of(id)%loc%grad%now(i,j,k)*p%of(id)%loc%delta%now(i,j,k)**2.0d0
	...
	!$omp parallel private(id,i,j,k,ii,jj,kk,a,b), num_threads(p%glb%threads)
