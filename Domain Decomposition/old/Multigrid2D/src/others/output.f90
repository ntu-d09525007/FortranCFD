...
use all
implicit none
		write(*,'("LS,  Loss of mass  (%) :",ES15.4)')(p%glb%mass-p%glb%imass)/p%glb%imass
		write(*,'("LS,  Loss of volume(%) :",ES15.4)')(p%glb%vol-p%glb%ivol)/p%glb%ivol
		write(*,*)''
		if(p%glb%method==3)then
			write(*,'("VOF, Loss of mass  (%) :",ES15.4)')(p%glb%massv-p%glb%imassv)/p%glb%imassv
			write(*,'("VOF, Loss of volume(%) :",ES15.4)')(p%glb%volv-p%glb%ivolv)/p%glb%ivolv
			write(*,*)''
		endif
end subroutine
