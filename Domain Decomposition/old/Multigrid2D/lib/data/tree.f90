...
real(8),dimension(:),allocatable :: B, id, i, j, k, ipiv
end type multigrid
...
real(8) :: dx,dy,dz
	...
	allocate(p%mg%i(p%mg%n),p%mg%j(p%mg%n),p%mg%k(p%mg%n),p%mg%node(p%mg%nx,p%mg%ny,p%mg%nz),p%mg%id(p%mg%n))
	...
	do i = 1, p%mg%n
		...
		ii = p%of(id)%loc%mg(level)%i(ind) + p%of(id)%loc%mg(level)%nx * p%of(id)%loc%idx 
		jj = p%of(id)%loc%mg(level)%j(ind) + p%of(id)%loc%mg(level)%ny * p%of(id)%loc%idy
		kk = p%of(id)%loc%mg(level)%k(ind) + p%of(id)%loc%mg(level)%nz * p%of(id)%loc%idz
		
		!write(*,'(I2,": ID=",I1," of ",I2," at (",I1,",",I1,")")')i,id,ind,ii,jj
	...
	!write(*,*)"I matrix",minval(p%mg%i),maxval(p%mg%i)
	!write(*,*)"J matrix",minval(p%mg%j),maxval(p%mg%j)
	!write(*,*)"K matrix",minval(p%mg%k),maxval(p%mg%k)
	!write(*,*)"Node matrix",minval(p%mg%node,3),maxval(p%mg%node,3)
...
error=0.0d0
!$omp parallel do private(id,ind,ii,jj,kk), num_threads(p%glb%threads), reduction(max:error)
do i = 1, p%mg%n
	id = (i-1) / p%of(0)%loc%mg(level)%n
	ind = i - id*p%of(0)%loc%mg(level)%n
	ii = p%of(id)%loc%mg(level)%i(ind)
	...
	!enddo
	!error = max(error, abs(tmp-p%of(id)%loc%mg(level)%src(ii,jj,kk)))
	p%of(id)%loc%mg(level)%sol(ii,jj,kk) = p%mg%B(i) 
enddo
!$omp end parallel do 

!write(*,*)"error of exact solver:",error
 ...
 do k = p%of(id)%loc%ks, p%of(id)%loc%ke
	...
	do j = p%of(id+iy*p%glb%grid_x)%loc%js, p%of(id+iy*p%glb%grid_x)%loc%je
		...
		do i = p%of(id+iy*p%glb%grid_x+ix)%loc%is, p%of(id+iy*p%glb%grid_x+ix)%loc%ie
			write(777,*)p%of(id+iy*p%glb%grid_x+ix)%loc%phi%now(i,j,k)
		enddo
		enddo
