...
type(time_recorded) :: q_cri, del_cri, lam2_cri, helicity
!-------------------------------------------
...
call p%loc%del_cri%alloc(is,ie,js,je,ks,ke)
call p%loc%lam2_cri%alloc(is,ie,js,je,ks,ke)

call p%loc%helicity%alloc(is,ie,js,je,ks,ke)
...
do level = 1, p%glb%level
	...
	if(mod(nx,2**(level-1)).eq.0 .and. nx/2**(level-1)>3)then
	...
	if(mod(ny,2**(level-1)).eq.0 .and. ny/2**(level-1)>3)then
	...
	!if(mod(nz,2**(level-1)).eq.0 .and. nz/2**(level-1)>3)then
...
real(8), dimension(p%loc%is-p%glb%ghc:p%loc%ie+p%glb%ghc,&
	...
	else if ( p%loc%idx == p%glb%grid_x-1 )then
	...
	else if (p%loc%idy==p%glb%grid_y-1)then
	...
	else if (p%loc%idz==p%glb%grid_z-1)then
