...
do
	...
	!$omp parallel private(id,i,j), num_threads(p%glb%threads), reduction(max:error)
		...
		do j = p%of(id)%loc%js, p%of(id)%loc%je
		do i = p%of(id)%loc%is, p%of(id)%loc%ie
			!p%of(id)%loc%phi%now(i,j) = 0.5_8 * p%of(id)%loc%phi%now(i,j) + 0.5_8 * p%of(id)%loc%phi%tmp(i,j)
...
real(8) :: grad
		...
		!$omp parallel private(id,i,j), reduction(max:grad), num_threads(p%glb%threads)
...
real(8) :: src
	...
	!$omp parallel private(id,i,j,src), num_threads(p%glb%threads)
		...
		do i = p%of(id)%loc%is, p%of(id)%loc%ie
			...
			p%of(id)%loc%srk6%x%l1(i,j) = (p%of(id)%loc%sign%tmp(i,j)-p%of(id)%loc%srk6%x%s1(i,j))*p%of(id)%loc%grad%now(i,j)-p%of(id)%loc%sign%tmp(i,j)
			
			src = p%of(id)%loc%srk6%x%l1(i,j)
	...
	!$omp parallel private(id,i,j,src), num_threads(p%glb%threads)
		...
		do i = p%of(id)%loc%is, p%of(id)%loc%ie
			...
			p%of(id)%loc%srk6%x%l2(i,j) = (p%of(id)%loc%sign%tmp(i,j)-p%of(id)%loc%srk6%x%s1(i,j))*p%of(id)%loc%grad%now(i,j)-p%of(id)%loc%sign%tmp(i,j)
			
			src = ( -3.0_8*p%of(id)%loc%srk6%x%l1(i,j)+p%of(id)%loc%srk6%x%l2(i,j) ) / 4.0_8
	...
	!$omp parallel private(id,i,j,src), num_threads(p%glb%threads)
		...
		do i = p%of(id)%loc%is, p%of(id)%loc%ie
			...
			p%of(id)%loc%srk6%x%l3(i,j) = (p%of(id)%loc%sign%tmp(i,j)-p%of(id)%loc%srk6%x%s1(i,j))*p%of(id)%loc%grad%now(i,j)-p%of(id)%loc%sign%tmp(i,j)
			
			src = ( -p%of(id)%loc%srk6%x%l1(i,j)-p%of(id)%loc%srk6%x%l2(i,j)+8.0_8*p%of(id)%loc%srk6%x%l3(i,j) ) / 12.0_8
...
real(8) :: upp,upm,ump,umm,vpp,vpm,vmp,vmm
	...
	!$omp parallel private(id,i,j,upp,upm,ump,umm,vpp,vpm,vmp,vmm), num_threads(p%glb%threads)
		...
		do j = p%of(id)%loc%js, p%of(id)%loc%je
			...
			call wenojs_flux(p%of(id)%loc%normals%x%now(:,j),p%of(id)%loc%srk6%x%s1(:,j),p%of(id)%loc%srk6%x%s2(:,j),&
		...
		do i = p%of(id)%loc%is, p%of(id)%loc%ie
			...
			call wenojs_flux(p%of(id)%loc%normals%y%now(i,:),p%of(id)%loc%srk6%y%s1(i,:),p%of(id)%loc%srk6%y%s2(i,:),&
		...
		do i = p%of(id)%loc%is, p%of(id)%loc%ie
			...
			upm=-MIN(p%of(id)%loc%srk6%x%s1(i,j),0.0_8)
			upp= MAX(p%of(id)%loc%srk6%x%s1(i,j),0.0_8)
			umm=-MIN(p%of(id)%loc%srk6%x%s2(i,j),0.0_8)
			ump= MAX(p%of(id)%loc%srk6%x%s2(i,j),0.0_8)
			
			vpm=-MIN(p%of(id)%loc%srk6%y%s1(i,j),0.0_8)
			vpp= MAX(p%of(id)%loc%srk6%y%s1(i,j),0.0_8)
			vmm=-MIN(p%of(id)%loc%srk6%y%s2(i,j),0.0_8)
			vmp= MAX(p%of(id)%loc%srk6%y%s2(i,j),0.0_8)
			
			if( p%of(id)%loc%sign%tmp(i,j) >= 0.0_8 )then
				p%of(id)%loc%grad%now(i,j) = dsqrt( MAX(upm,ump)**2 + MAX(vpm,vmp)**2 )
			else 
				p%of(id)%loc%grad%now(i,j) = dsqrt( MAX(upp,umm)**2 + MAX(vpp,vmm)**2 )
			end if
...
real(8) :: a,b,dv
	...
	if( btn==0 )then
		...
		!$omp parallel private(id,i,j,ii,jj,a,b), num_threads(p%glb%threads)
			...
			do j = p%of(id)%loc%js, p%of(id)%loc%je
			do i = p%of(id)%loc%is, p%of(id)%loc%ie	
				p%of(id)%loc%srk6%x%s1(i,j) = 0.0d0		
			end do
			end do 
	...
	dv=(p%glb%ivol-p%glb%vol)/p%glb%rdt
	...
	!$omp parallel private(id,i,j), num_threads(p%glb%threads)
		...
		do i = p%of(id)%loc%is, p%of(id)%loc%ie
			...
			p%of(id)%loc%srk6%x%s2(i,j) = p%of(id)%loc%delta%now(i,j)*p%of(id)%loc%sign%tmp(i,j)*( p%of(id)%loc%grad%now(i,j) - 1.0d0 ) 
			p%of(id)%loc%srk6%x%s3(i,j) = p%of(id)%loc%grad%now(i,j)*p%of(id)%loc%delta%now(i,j)**2.0d0
	...
	!$omp parallel private(id,i,j,ii,jj,a,b), num_threads(p%glb%threads)
		...
		do i = p%of(id)%loc%is, p%of(id)%loc%ie
			...
			a = 15.0_8*p%of(id)%loc%srk6%x%s2(i,j)
			b = 15.0_8*p%of(id)%loc%srk6%x%s3(i,j)
				
			do ii = -1, 1
			do jj = -1, 1
				a = a + p%of(id)%loc%srk6%x%s2(i+ii,j+jj)
				b = b + p%of(id)%loc%srk6%x%s3(i+ii,j+jj)
			end do
			end do 

			p%of(id)%loc%srk6%x%s1(i,j) = 0.0d0
			if( abs(b)>1.0d-12 )p%of(id)%loc%srk6%x%s1(i,j) = a/b*p%of(id)%loc%delta%now(i,j)
