...
type plotter_data
integer :: is, ie
real(8), dimension(:,:), pointer :: dat, x, y
end type
...
integer :: id, i
	...
	this%pthreads = src%glb%pthreads
...
class(plotter) :: p
type(manager) :: src
integer :: id,s,e,nx,ny,i
character(6) :: name

	if ( abs(p%src%glb%time - p%src%glb%pid * p%src%glb%t2p) > p%src%glb%dt ) return
	
	!$omp parallel private(id,name,s,e,nx,ny,i), num_threads(p%pthreads)
		...
		WRITE(id+777,'(A,3I6)')"DIMENSIONS ",nx,ny,1
		
		WRITE(id+777,'(A,3ES15.4)')"SPACING ",p%src%glb%dx, p%src%glb%dy, 0.0_8
		
		IF( ID/=0 )then
			WRITE(id+777,'(A,3ES15.4)')"ORIGIN ",p%src%glb%x(p%of(id)%from(s)%is-1),p%src%glb%ystart,0.0d0
		ELSE
			WRITE(id+777,'(A,3ES15.4)')"ORIGIN ",p%src%glb%x(p%of(id)%from(s)%is-1)-0.5D0*p%src%glb%DX,p%src%glb%ystart,0.0d0
		ENDIF		
		WRITE(id+777,'(A,3I6)')"POINT_DATA ",nx*ny
		...
		do i = s, e
			p%of(id)%from(i)%dat => p%src%of(i)%loc%heavy%now
		enddo
		
		call p%of(id)%plot_scalar(id+777,"H")
...
character(*) :: name
	...
	do j = 0, p%ny
		...
		write(fid,*)p%from(p%s)%dat(p%from(p%s)%is-1,j)
		
		do m = p%s, p%e
		do i = p%from(m)%is, p%from(m)%ie
			write(fid,*)p%from(m)%dat(i,j)
		enddo
		enddo
...
character(*) :: name
	...
	do j = 0, p%ny
		...
		write(fid,*)p%from(p%s)%x(p%from(p%s)%is-1,j),p%from(p%s)%y(p%from(p%s)%is-1,j),0.0_8
		
		do m = p%s, p%e
		do i = p%from(m)%is, p%from(m)%ie
			write(fid,*)p%from(m)%x(i,j),p%from(m)%y(i,j),0.0_8
		enddo
		enddo
