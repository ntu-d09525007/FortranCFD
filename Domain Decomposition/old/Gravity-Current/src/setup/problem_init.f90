...
CHARACTER(100) :: NAME_OF_FILE
    ...
    ug=30
    !$omp parallel private(id,i,j,k,ii,jj,kk,x,y,z), num_threads(p%glb%threads)
        ...
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
			...
			! Gravity Current
            !=========================================
            !if( x<=2.0d0 .and. y<=2.0d0 .and. z<=2.0d0 )then
            !    p%of(id)%loc%phi%now(i,j,k) = 1.0_8
            !else
            !    p%of(id)%loc%phi%now(i,j,k) = -1.0_8
            !end if
			
			p%of(id)%loc%phi%now(i,j,k) = -x+2.0d0
	...
	!$omp parallel private(id,i,j,k), num_threads(p%glb%threads)	
        ...
        do i = p%of(id)%loc%is-p%glb%ghc, p%of(id)%loc%ie+p%glb%ghc		
			p%of(id)%loc%phi%now(i,j,k) = p%of(id)%loc%heavy%now(i,j,k)		
		enddo
		enddo
		enddo		
	!$omp end parallel
