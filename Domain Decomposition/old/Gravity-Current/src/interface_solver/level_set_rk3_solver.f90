...
!$omp parallel private(id,i,j,k), num_threads(p%glb%threads)
    ...
    if( p%of(id)%loc%idx==0 )then
        ...
        do k = p%of(id)%loc%ks, p%of(id)%loc%ke
        do j = p%of(id)%loc%js, p%of(id)%loc%je
            p%of(id)%loc%phi%now(1,j,k) = ( 9.0d0*p%of(id)%loc%phi%now(2,j,k)-p%of(id)%loc%phi%now(3,j,k) )/8.0d0
        enddo
        enddo
    
    else if ( p%of(id)%loc%idx==p%glb%grid_x-1 )then
        
        do k = p%of(id)%loc%ks, p%of(id)%loc%ke
        do j = p%of(id)%loc%js, p%of(id)%loc%je
            p%of(id)%loc%phi%now(p%glb%node_x,j,k) = ( 9.0d0*p%of(id)%loc%phi%now(p%glb%node_x-1,j,k) - &
            & p%of(id)%loc%phi%now(p%glb%node_x-2,j,k) )/8.0d0
        enddo
        enddo
    ...
    if( p%of(id)%loc%idy==0 )then
        ...
        do k = p%of(id)%loc%ks, p%of(id)%loc%ke
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
            p%of(id)%loc%phi%now(i,1,k) = ( 9.0d0*p%of(id)%loc%phi%now(i,2,k)-p%of(id)%loc%phi%now(i,3,k) )/8.0d0
        enddo
        enddo
    
    else if ( p%of(id)%loc%idy==p%glb%grid_y-1 )then
        
        do k = p%of(id)%loc%ks, p%of(id)%loc%ke
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
            p%of(id)%loc%phi%now(i,p%glb%node_y,k) = ( 9.0d0*p%of(id)%loc%phi%now(i,p%glb%node_y-1,k) - &
            & p%of(id)%loc%phi%now(i,p%glb%node_y-2,k) )/8.0d0
        enddo
        enddo
    ...
    if( p%of(id)%loc%idz==0 )then
        ...
        do j = p%of(id)%loc%js, p%of(id)%loc%je
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
            p%of(id)%loc%phi%now(i,j,1) = ( 9.0d0*p%of(id)%loc%phi%now(i,j,2)-p%of(id)%loc%phi%now(i,j,3) )/8.0d0
        enddo
        enddo
    
    else if ( p%of(id)%loc%idz==p%glb%grid_z-1 )then
        
        do j = p%of(id)%loc%js, p%of(id)%loc%je
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
            p%of(id)%loc%phi%now(i,j,p%glb%node_z) = ( 9.0d0*p%of(id)%loc%phi%now(i,j,p%glb%node_z-1) - &
            & p%of(id)%loc%phi%now(i,j,p%glb%node_z-2) )/8.0d0
        enddo
        enddo
...
!$omp parallel private(id,i,j,k,src), num_threads(p%glb%threads)
    ...
    if( p%of(id)%loc%idz==0 )then
        ...
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
        do j = p%of(id)%loc%js, p%of(id)%loc%je
            src = p%glb%us*(p%of(id)%loc%phi%now(i,j,2)-p%of(id)%loc%phi%now(i,j,1))/p%glb%dz*p%glb%dt
            p%of(id)%loc%phi%now(i,j,1) = p%of(id)%loc%phi%now(i,j,1) + src
        enddo
        enddo
        ...
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
        do j = p%of(id)%loc%js, p%of(id)%loc%je
            p%of(id)%loc%phi%now(i,j,p%glb%node_z) = p%of(id)%loc%phi%now(i,j,p%glb%node_z-1) / ( -p%glb%Re*p%glb%Dz*p%glb%us + 1.0d0 )
        enddo
        enddo
...
!$omp parallel private(id,i,j,k), num_threads(p%glb%threads)
    ...
    do j = p%of(id)%loc%js, p%of(id)%loc%je
    do i = p%of(id)%loc%is, p%of(id)%loc%ie
        p%of(id)%loc%phi%now(i,j,k) = max(min(1.0d0,p%of(id)%loc%phi%now(i,j,k)),0.0d0)
    enddo
    enddo
    enddo
...
integer :: id,i,j,k
    ...
    !$omp parallel private(id,i,j,k), num_threads(p%glb%threads)
        ...
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
            ...
            p%of(id)%loc%nvel%z%old(i,j,k) = 0.5d0*( p%of(id)%loc%vel%z%old(i,j,k-1)+p%of(id)%loc%vel%z%old(i,j,k) ) - p%glb%us
        ...
        call p%of(id)%nvelbc(p%of(id)%loc%nvel%x%old,p%of(id)%loc%nvel%y%old,p%of(id)%loc%nvel%z%old)
    
    !$omp end parallel 
    
    call pt%nvel_old%sync
...
integer :: i,j,k
    ...
    do i = q%loc%is, q%loc%ie
        ...
        s(i,j,k) = - q%loc%nvel%x%old(i,j,k)*q%loc%tdata%x%s1(i,j,k) + 1.0d0/q%glb%re * q%loc%tdata%x%s2(i,j,k) &
                ...
                &  - q%loc%nvel%z%old(i,j,k)*q%loc%tdata%z%s1(i,j,k) + 1.0d0/q%glb%re * q%loc%tdata%z%s2(i,j,k)
    end do 
    end do
    end do
