...
subroutine multigrid_final_exact_iterative(num)
use all
implicit none
integer :: id,level,relax_iter,num
integer :: i,j,k
real(8) :: dx,dy,dz,a,w,pnew,rms0
...
call multigrid_residual(level,.false.)
rms0 = p%of(0)%loc%mg(level)%l2norm
...
!$omp parallel private(id,i,j,k,dx,dy,dz), num_threads(p%glb%threads), reduction(+:l2norm)
...
!$omp parallel private(id,i,j,k), num_threads(p%glb%threads), reduction(+:s)   
