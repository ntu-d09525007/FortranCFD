...
do iter = 1, 2000
    ...
    do level = 1, p%glb%level-1
        ...
        !write(*,*)level,p%of(0)%loc%mg(level)%l2norm
    enddo
    
    call multigrid_final_exact()
    !write(*,*)p%glb%level,p%of(0)%loc%mg(p%glb%level)%l2norm
...
real(8) :: src
    ...
    !$omp parallel private(id,i,j,k,src), num_threads(p%glb%threads)
        ...
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
            ...
            src = ( p%of(id)%loc%p%now(i,j+1,k) - p%of(id)%loc%p%now(i,j,k) )/p%glb%dx
            ...
            src = ( p%of(id)%loc%p%now(i,j,k+1) - p%of(id)%loc%p%now(i,j,k) )/p%glb%dx
