    ...
    do id = 0, p%glb%threads-1
        ...
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
            ...
            do kk = 1, ug
                ...
                if(z<=-2 .and. x**2+y**2+(z+3.2)**2>1.0)then
            ...
            if(z<-2.0)then
                if(x**2+y**2+(z+3.2)**2<1.0)then
                    p%of(id)%loc%phi%now(i,j,k) = dsqrt( x**2+y**2+(z+3.2)**2 ) - 1.0
                else
                    p%of(id)%loc%phi%now(i,j,k) = min( dsqrt( x**2+y**2+(z+3.2)**2 ) - 1.0, -z-2.0 )
            else
                p%of(id)%loc%phi%now(i,j,k) = -z-2.0
            endif
