...
integer :: id,i,j
    ...
    do id = 0, p%glb%threads-1
        ...
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
            ...
            ! if( k==1 )then
            !     p%of(id)%loc%coe%c(i,j)=p%of(id)%loc%coe%c(i,j)+p%of(id)%loc%coe%d(i,j)
            !     p%of(id)%loc%coe%d(i,j)=0.0d0
            ! endif
            
            ! if( k==p%glb%node_z )then
            !     p%of(id)%loc%coe%c(i,j)=p%of(id)%loc%coe%c(i,j)+p%of(id)%loc%coe%u(i,j)
            !     p%of(id)%loc%coe%u(i,j)=0.0d0
            ! endif
