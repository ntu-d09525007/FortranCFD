...
integer :: id,i,j
    ...
    do id = 0, p%glb%threads-1
        ...
        do j = p%of(id)%loc%js, p%of(id)%loc%je
            ...
            ! ux ------------------------------------------------------------------------
            call p%of(id)%loc%ccdsolvers%x%solve("uccd",p%of(id)%loc%tdata%x%s1(:,j),&
                                                &p%of(id)%loc%tdata%x%ss1(:,j),&
                                                &p%of(id)%loc%vel%x%tmp(:,j),&
                                                &p%of(id)%loc%vel%x%old(:,j))   

            call p%of(id)%loc%ccdsolvers%x%solve("uccd",p%of(id)%loc%tdata%x%s2(:,j),&
                                                &p%of(id)%loc%tdata%x%ss2(:,j),&
                                                &p%of(id)%loc%vel%x%tmp(:,j),&
                                                &p%of(id)%loc%vel%x%old(:,j)) 
                                                
            call p%of(id)%loc%ccdsolvers%x%solve("uccd",p%of(id)%loc%tdata%x%s3(:,j),&
                                                &p%of(id)%loc%tdata%x%ss3(:,j),&
                                                &p%of(id)%loc%vel%x%tmp(:,j),&
                                                &p%of(id)%loc%vel%x%old(:,j)) 
            
            ! vx ------------------------------------------------------------------------
            call p%of(id)%loc%ccdsolvers%x%solve("uccd",p%of(id)%loc%tdata%y%s1(:,j),&
                                                &p%of(id)%loc%tdata%y%ss1(:,j),&
                                                &p%of(id)%loc%vel%x%tmp(:,j),&
                                                &p%of(id)%loc%velsrc%x%now(:,j))

            call p%of(id)%loc%ccdsolvers%x%solve("uccd",p%of(id)%loc%tdata%y%s2(:,j),&
                                                &p%of(id)%loc%tdata%y%ss2(:,j),&
                                                &p%of(id)%loc%vel%x%tmp(:,j),&
                                                &p%of(id)%loc%velsrc%x%now(:,j))

            call p%of(id)%loc%ccdsolvers%x%solve("uccd",p%of(id)%loc%tdata%y%s3(:,j),&
        ...
        do i = p%of(id)%loc%is, p%of(id)%loc%ie
            ...
            ! uy ------------------------------------------------------------------------
            call p%of(id)%loc%ccdsolvers%y%solve("uccd",p%of(id)%loc%tdata%x%s1(i,:),&
                                                &p%of(id)%loc%tdata%x%ss1(i,:),&
                                                &p%of(id)%loc%vel%y%tmp(i,:),&
                                                &p%of(id)%loc%velsrc%y%now(i,:))

            call p%of(id)%loc%ccdsolvers%y%solve("uccd",p%of(id)%loc%tdata%x%s2(i,:),&
                                                &p%of(id)%loc%tdata%x%ss2(i,:),&
                                                &p%of(id)%loc%vel%y%tmp(i,:),&
                                                &p%of(id)%loc%velsrc%y%now(i,:))

            call p%of(id)%loc%ccdsolvers%y%solve("uccd",p%of(id)%loc%tdata%x%s3(i,:),&
                                                &p%of(id)%loc%tdata%x%ss3(i,:),&
                                                &p%of(id)%loc%vel%y%tmp(i,:),&
                                                &p%of(id)%loc%velsrc%y%now(i,:))

            ! vy ------------------------------------------------------------------------
            call p%of(id)%loc%ccdsolvers%y%solve("uccd",p%of(id)%loc%tdata%y%s1(i,:),&
                                                &p%of(id)%loc%tdata%y%ss1(i,:),&
                                                &p%of(id)%loc%vel%y%tmp(i,:),&
                                                &p%of(id)%loc%vel%y%old(i,:))

            call p%of(id)%loc%ccdsolvers%y%solve("uccd",p%of(id)%loc%tdata%y%s2(i,:),&
                                                &p%of(id)%loc%tdata%y%ss2(i,:),&
                                                &p%of(id)%loc%vel%y%tmp(i,:),&
                                                &p%of(id)%loc%vel%y%old(i,:))

            call p%of(id)%loc%ccdsolvers%y%solve("uccd",p%of(id)%loc%tdata%y%s3(i,:),&
...
subroutine ns_split_diff_store_vel
use all
implicit none
integer :: id,i,j
...
call ns_split_diff_store_vel

do iter = 1, 5

call ns_linearize

! ----------------------------- Crank-Nicolson Part 1
!$omp parallel do 
do id = 0, p%glb%threads-1
    call p%of(id)%find_stag_vel( p%of(id)%loc%velsrc%x%now, p%of(id)%loc%velsrc%y%now, &
...
! ----------------------------- Crank-Nicolson Part 2
!$omp parallel do 
do id = 0, p%glb%threads-1
    call p%of(id)%find_stag_vel( p%of(id)%loc%velsrc%x%now, p%of(id)%loc%velsrc%y%now, &
                                &p%of(id)%loc%vel%x%old2   , p%of(id)%loc%vel%y%old2 )
enddo
!$omp end parallel do
...
subroutine ns_split_diff_source(q,u,v,us,vs,sx,sy,irho,imu,idelta,icurv,reset,w)
use all
implicit none
type(job) :: q
real(8), dimension(q%loc%is-q%glb%ghc:q%loc%ie+q%glb%ghc,&
                  &q%loc%js-q%glb%ghc:q%loc%je+q%glb%ghc) :: u,v,us,vs,sx,sy,irho,imu,idelta,icurv,tmp
...
do j = q%loc%js, q%loc%je
    call q%loc%ccdsolvers%x%solve("uccd",us(:,j),q%loc%tdata%x%s1(:,j),q%loc%tdata%x%ss1(:,j),us(:,j))
    call q%loc%ccdsolvers%x%solve("uccd",vs(:,j),q%loc%tdata%y%s1(:,j),q%loc%tdata%y%ss1(:,j),u(:,j))

    call q%loc%ccdsolvers%x%solve("uccd",v(:,j),q%loc%tdata%y%l1(:,j),tmp(:,j),us(:,j))
end do

do i = q%loc%is, q%loc%ie
    call q%loc%ccdsolvers%y%solve("uccd",us(i,:),q%loc%tdata%x%s2(i,:),q%loc%tdata%x%ss2(i,:),us(i,:))
    call q%loc%ccdsolvers%y%solve("uccd",vs(i,:),q%loc%tdata%y%s2(i,:),q%loc%tdata%y%ss2(i,:),vs(i,:))

    call q%loc%ccdsolvers%y%solve("uccd",u(i,:),q%loc%tdata%x%l1(i,:),tmp(i,:),vs(i,:))
end do
