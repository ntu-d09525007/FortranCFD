...
integer :: id, is, ie
real(kind=ap) :: dx, dt
type(SRKCCDC_1D) :: srkccdc
type(SRKCCD_1D) :: srkccd
type(SRKUCCD_1D) :: srkuccd
contains
...
real(kind=ap) :: dx,dt
 ...
 call this%srkccd%init(is,ie,dx,dt)
 call this%srkccdc%init(is,ie,dx,dt)
 call this%srkuccd%init(is,ie,dx,dt)
...
implicit none
class(my_solver) :: this
real(kind=ap), dimension(this%is-3:this%ie+3) :: u, f, s
integer :: i

if( this%id.eq.1 )then
	call this%srkccd%solve(u,f,s)
else if (this%id.eq.2 )then
	call this%srkuccd%solve(u,f,s)
else if (this%id.eq.3 )then
	call this%srkccdc%solve(u,f,s)
endif 
...
this%cfl = 0.5_ap !+ 0.2_ap*(id-1)

this%turn = 0.05
this%t2s = 2.4_AP
this%t2p = 0.4!0.2_ap*this%t2s
this%t = 0.0_ap
this%t_shock = 2.0_ap
...
subroutine set_value(this,u,t)
use my_precision
implicit none
class(pure_adv) :: this
real(kind=ap),dimension(-2:this%nx+3) :: u
integer :: i
real(kind=ap) :: x,t,err,unew

do

 err=0.0_ap
 
 !$omp parallel do private(x,unew), reduction(max:err)
 do i = 1, this%nx
	 x = this%x(i)-u(i)*t
	 unew = sin(2.0*acos(-1.0_ap)*x) / (2.0_ap*acos(-1.0_ap)*this%t_shock)
	 err = max(err,abs(unew-u(i)))
	 u(i) = unew
 enddo
 !$omp end parallel do
...
real(kind=ap), dimension(3) :: err1, err2, err3
 ...
 do
	...
	call this%solver%solve(this%u1,this%u1,this%s1)
	call this%solver%solve(this%u2,this%u2,this%s2)
	call this%solver%solve(this%u3,this%u3,this%s3)
...
real(kind=ap) :: maxc
 ...
 !$omp parallel do reduction(max:maxc)
 do i = 1, this%nx
	 maxc = max(maxc,abs(this%u(i)))
 enddo
 !$omp end parallel do
 
 this%dt = this%cfl * this%dx / maxc
