...
real(8) :: src
    ...
    !$omp parallel do private(src), collapse(2)
    do j = p%loc%js, p%loc%je
    do i = p%loc%is, p%loc%ie
        src = p%loc%tdata%x%l1(i,j)
        p%loc%phi%now(i,j) = p%loc%phi%now(i,j) + src * p%glb%dt
    end do 
    end do 
    !$omp end parallel do 
    ...
    do j = p%loc%js, p%loc%je
    do i = p%loc%is, p%loc%ie
        src = ( -3.0d0*p%loc%tdata%x%l1(i,j) + p%loc%tdata%x%l2(i,j) ) / 4.0d0
        p%loc%phi%now(i,j) = p%loc%phi%now(i,j) + src * p%glb%dt
    end do 
    end do
    !$omp end parallel do 
    ...
    do j = p%loc%js, p%loc%je
    do i = p%loc%is, p%loc%ie
        src = ( -p%loc%tdata%x%l1(i,j)-p%loc%tdata%x%l2(i,j)+8.0d0*p%loc%tdata%x%l3(i,j) ) / 12.0d0
        p%loc%phi%now(i,j) = p%loc%phi%now(i,j) + src * p%glb%dt
    end do 
    end do 
    !$omp end parallel do 
...
do i = p%loc%is, p%loc%ie
    ...
    p%loc%tdata%x%s1(i,j) = 0.5d0*(p%loc%nvel%x%old(i,j)+abs(p%loc%nvel%x%old(i,j)))*p%loc%phi%now(i,j)
    p%loc%tdata%x%s2(i,j) = 0.5d0*(p%loc%nvel%x%old(i,j)-abs(p%loc%nvel%x%old(i,j)))*p%loc%phi%now(i,j)

    p%loc%tdata%y%s1(i,j) = 0.5d0*(p%loc%nvel%y%old(i,j)+abs(p%loc%nvel%y%old(i,j)))*p%loc%phi%now(i,j)
    p%loc%tdata%y%s2(i,j) = 0.5d0*(p%loc%nvel%y%old(i,j)-abs(p%loc%nvel%y%old(i,j)))*p%loc%phi%now(i,j)
...
real(8), dimension(p%loc%is-p%glb%ghc:p%loc%ie+p%glb%ghc,&
                  &p%loc%js-p%glb%ghc:p%loc%je+p%glb%ghc) :: s
integer :: i,j,k
