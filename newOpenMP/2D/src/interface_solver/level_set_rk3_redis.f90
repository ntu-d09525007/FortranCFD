...
do
    ...
    do j = p%loc%js, p%loc%je
    do i = p%loc%is, p%loc%ie
        error = max( error, abs(p%loc%phi%tmp(i,j)-p%loc%phi%now(i,j)) )
    end do 
    end do
    !$omp end parallel do 
...
do j = p%loc%js, p%loc%je
do i = p%loc%is, p%loc%ie
    p%loc%sign%tmp(i,j) = p%loc%sign%now(i,j)
end do 
end do
!$omp end parallel do
...
do j = p%loc%js, p%loc%je
do i = p%loc%is, p%loc%ie
    p%loc%sign%tmp(i,j) = p%loc%phi%now(i,j)
end do 
end do
!$omp end parallel do 
...
grad = 0.0_8
!$omp parallel do collapse(2), reduction(max:grad)
do j = p%loc%js, p%loc%je
do i = p%loc%is, p%loc%ie
    grad = max( grad, p%loc%grad%now(i,j) )
end do 
end do
!$omp end parallel do
...
do j = p%loc%js, p%loc%je
do i = p%loc%is, p%loc%ie         
    p%loc%phi%now(i,j) = p%loc%phi%now(i,j) / grad
end do
end do
!$omp end parallel do 
...
do i = p%loc%is, p%loc%ie
    ...
    p%loc%tdata%x%l1(i,j) = (p%loc%sign%tmp(i,j)-p%loc%tdata%x%s1(i,j))*p%loc%grad%now(i,j)-p%loc%sign%tmp(i,j)
    
    src = p%loc%tdata%x%l1(i,j)

    p%loc%phi%now(i,j) = p%loc%phi%now(i,j) - p%glb%rdt * src
...
do i = p%loc%is, p%loc%ie
    ...
    p%loc%tdata%x%l2(i,j) = (p%loc%sign%tmp(i,j)-p%loc%tdata%x%s1(i,j))*p%loc%grad%now(i,j)-p%loc%sign%tmp(i,j)
    
    src = ( -3.0_8*p%loc%tdata%x%l1(i,j)+p%loc%tdata%x%l2(i,j) ) / 4.0_8
...
do i = p%loc%is, p%loc%ie
    ...
    p%loc%tdata%x%l3(i,j) = (p%loc%sign%tmp(i,j)-p%loc%tdata%x%s1(i,j))*p%loc%grad%now(i,j)-p%loc%sign%tmp(i,j)
    
    src = ( -p%loc%tdata%x%l1(i,j)-p%loc%tdata%x%l2(i,j)+8.0_8*p%loc%tdata%x%l3(i,j) ) / 12.0_8
...
!$omp parallel do collapse(2), private(upp,upm,ump,umm,vpp,vpm,vmp,vmm)
do j = p%loc%js, p%loc%je
do i = p%loc%is, p%loc%ie
    
    upm=-MIN(p%loc%tdata%x%s1(i,j),0.0_8)
    upp= MAX(p%loc%tdata%x%s1(i,j),0.0_8)
    umm=-MIN(p%loc%tdata%x%s2(i,j),0.0_8)
    ump= MAX(p%loc%tdata%x%s2(i,j),0.0_8)
    
    vpm=-MIN(p%loc%tdata%y%s1(i,j),0.0_8)
    vpp= MAX(p%loc%tdata%y%s1(i,j),0.0_8)
    vmm=-MIN(p%loc%tdata%y%s2(i,j),0.0_8)
    vmp= MAX(p%loc%tdata%y%s2(i,j),0.0_8)
    
    if( p%loc%sign%tmp(i,j) >= 0.0_8 )then
        p%loc%grad%now(i,j) = dsqrt( MAX(upm,ump)**2.0d0 + MAX(vpm,vmp)**2.0d0  )
    else 
        p%loc%grad%now(i,j) = dsqrt( MAX(upp,umm)**2.0d0 + MAX(vpp,vmm)**2.0d0  )
    end if
...
!$omp parallel do collapse(2), private(lam)  
do j = p%loc%js, p%loc%je
do i = p%loc%is, p%loc%ie
    
    lam = p%loc%delta%now(i,j)
    !lam = (2.0d0*(1.0d0-p%glb%rho_12)*p%loc%heavy%now(i,j)+p%glb%rho_12)*p%loc%delta%now(i,j)
    
    p%loc%tdata%x%s2(i,j) = lam*p%loc%sign%tmp(i,j)*( p%loc%grad%now(i,j) - 1.0d0 ) 
    p%loc%tdata%x%s3(i,j) = p%loc%grad%now(i,j)*p%loc%delta%now(i,j)*lam
...
do i = p%loc%is, p%loc%ie
    ...
    a = 16.0d0*p%loc%tdata%x%s2(i,j)
    b = 16.0d0*p%loc%tdata%x%s3(i,j)
        
    do jj = -1, 1
    do ii = -1, 1
        a = a + p%loc%tdata%x%s2(i+ii,j+jj)
        b = b + p%loc%tdata%x%s3(i+ii,j+jj)
    end do 
    end do

    p%loc%tdata%x%s1(i,j) = 0.0d0
    if( abs(b)>1.0d-12 )p%loc%tdata%x%s1(i,j) = a/b*p%loc%delta%now(i,j)
