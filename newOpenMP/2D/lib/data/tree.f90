...
real(8) :: mag
    ...
    call p%loc%alloc(p%glb%node_x,p%glb%node_y,p%glb%ghc,&
                            p%glb%dx,p%glb%dy,p%glb%dt,p%glb%t_w)
...
real(8) :: x,y,z
 ...
 WRITE(777,'(A,3I6)')"DIMENSIONS ",p%glb%node_x,p%glb%node_y,1
 WRITE(777,'(A,3ES15.4)')"SPACING ",p%glb%dx, p%glb%dy, 0.0d0  
 WRITE(777,'(A,3ES15.4)')"ORIGIN ",p%glb%x(1,1),p%glb%y(1,1),0.0d0
 WRITE(777,'(A,I12)')"POINT_DATA ",(p%glb%node_x)*(p%glb%node_y)
