...
REAL(DP) :: ux,uy,uz,vx,wx,phix,phiy,phiz
 ...
 !$omp parallel do private(up,um,vv,vp,vm,ww,wp,wm,cm,cr,cc,detf,curvk,finx,a1,a2,a3,a4,a5,a6,a7,TMP,ux,uy,uz,vx,wx,phix,phiy,phiz)
 DO K = 1, NODE_Z
 DO J = 1, NODE_Y
 DO I = 1, NODE_X-1
    ...
    TMP = HEAV(0.5_DP*(PHI_old(I,J,K)+PHI_old(I+1,J,K)),DETF)
    ...
    A2=0.5_DP/DY*(VP*(3.0_DP*U_old(I,J,K)-4.0_DP*U_old(I,J-1,K)+U_old(I,J-2,K)) &
                 +VM*(-U(I,J+2,K)+4.0_DP*U_old(I,J+1,K)-3.0_DP*U_old(I,J,K)))
    ...
    !US(I,J,K)=-A4-A5-A6+A7
    
    ux = 0.5d0*( u_old(i+1,j,k)-u_old(i-1,j,k) )/dx
    uy = 0.5d0*( u_old(i,j+1,k)-u_old(i,j-1,k) )/dy
    uz = 0.5d0*( u_old(i,j,k+1)-u_old(i,j,k-1) )/dz
    
    vx = 0.5d0*( v_old(i+1,j,k)-v_old(i,j,k)+v_old(i+1,j-1,k)-v_old(i,j-1,k) )/dx
    wx = 0.5d0*( w_old(i+1,j,k)-w_old(i,j,k)+w_old(i+1,j,k-1)-w_old(i,j,k-1) )/dx
    
    phix = (phi_old(i+1,j,k)-phi_old(i,j,k))/dx
    phiy = 0.25d0*(phi_old(i+1,j+1,k)-phi_old(i+1,j-1,k)+phi_old(i,j+1,k)-phi_old(i,j-1,k))/dy
    phiz = 0.25d0*(phi_old(i+1,j,k+1)-phi_old(i+1,j,k-1)+phi_old(i,j,k+1)-phi_old(i,j,k-1))/dz
    
    US(i,j,k) = US(i,j,k) - 2.0d0*(1.0d0-RATIO_RHO)*detf*( phix*(2.0d0*ux) + phiy*(uy+vx) + phiz*(uz+wx) ) / (CR*RE)
 ...
 real(dp) :: phix,phiy,phiz,vx,vy,vz,uy,wy
 ...
 !$omp parallel do private(UU,up,um,vp,vm,ww,wp,wm,cm,cc,cr,detf,finy,curvk,b1,b2,b3,b4,b5,b6,b7,TMP,phix,phiy,phiz,vx,vy,vz,uy,wy)
 DO K = 1, NODE_Z
 DO J = 1, NODE_Y-1
 DO I = 1, NODE_X
    ...
    TMP = HEAV(0.5_DP*(PHI_old(I,J,K)+PHI_old(I,J+1,K)),DETF)
    ...
    !VS(I,J,K)=-B4-B5-B6+b7
    
    vx = 0.5d0*( v_old(i+1,j,k)-v_old(i-1,j,k) )/dx
    vy = 0.5d0*( v_old(i,j+1,k)-v_old(i,j-1,k) )/dy
    vz = 0.5d0*( v_old(i,j,k+1)-v_old(i,j,k-1) )/dz
    
    uy = 0.5d0*( u_old(i,j+1,k)-u_old(i,j,k)+u_old(i-1,j+1,k)-u_old(i-1,j,k) )/dy
    wy = 0.5d0*( w_old(i,j+1,k)-w_old(i,j,k)+w_old(i,j+1,k-1)-w_old(i,j,k-1) )/dy
    
    phix = 0.25d0*(phi_old(i+1,j,k)-phi_old(i-1,j,k)+phi_old(i+1,j+1,k)-phi_old(i-1,j+1,k))/dx
    phiy = ( phi_old(i,j+1,k)-phi_old(i,j,k) )/dy
    phiz = 0.25d0*(phi_old(i,j+1,k+1)-phi_old(i,j+1,k-1)+phi_old(i,j,k+1)-phi_old(i,j,k-1))/dz
    
    vs(i,j,k) = vs(i,j,k) - 2.0d0*(1.0d0-RATIO_RHO)*detf*( phix*(uy+vx) + phiy*(2.0d0*vy) + phiz*(vz+wy) ) / (CR*RE)
...
SUBROUTINE VELC()
 ...
 real(dp) :: phix,phiy,phiz,wx,wy,wz,uz,vz
 ...
 !$omp parallel do private(uu,up,um,vv,vp,vm,wp,wm,cm,cr,cc,detf,curvk,finz,c1,c2,c3,c4,c5,c6,c7,TMP,phix,phiy,phiz,wx,wy,wz,uz,vz)
 DO K = 1, NODE_Z-1
 DO J = 1, NODE_Y
 DO I = 1, NODE_X
    ...
    TMP = HEAV(0.5_DP*(PHI_old(I,J,K)+PHI_old(I,J,K+1)),DETF)
    ...
    !WS(I,J,K)=-C4-C5-C6+C7
  
    IF( G_FORCE==1 )WS(I,J,K)=WS(I,J,K)+1.0_DP/FR2
    
    wx = 0.5d0*( w_old(i+1,j,k)-w_old(i-1,j,k) )/dx
    wy = 0.5d0*( w_old(i,j+1,k)-w_old(i,j-1,k) )/dy
    wz = 0.5d0*( w_old(i,j,k+1)-w_old(i,j,k-1) )/dz
    
    uz = 0.5d0*( u_old(i,j,k+1)-u_old(i,j,k)+u_old(i-1,j,k+1)-u_old(i-1,j,k) )/dz
    vz = 0.5d0*( v_old(i,j,k+1)-v_old(i,j,k)+v_old(i,j-1,k+1)-v_old(i,j-1,k) )/dz
    
    phix = 0.25d0*( phi_old(i+1,j,k+1)-phi_old(i-1,j,k+1) + phi_old(i+1,j,k)-phi_old(i-1,j,k) )/dx
    phiy = 0.25d0*( phi_old(i,j+1,k+1)-phi_old(i,j-1,k+1) + phi_old(i,j+1,k)-phi_old(i,j-1,k) )/dy
    phiz = ( phi_old(i,j,k+1)-phi_old(i,j,k) )/dz
    
    ws(i,j,k) = ws(i,j,k) - 2.0d0*(1.0d0-RATIO_RHO)*detf*( phix*(uz+wx) + phiy*(vz+wy) + phiz*(2.0d0*wz) ) / (CR*RE)
