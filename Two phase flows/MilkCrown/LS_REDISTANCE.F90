...
END IF
  ...
  DO
    ...
    IF( BTN==0 .AND. (RED_TIME > MAX(XL,YL,ZL)) )EXIT
    IF( BTN/=0 .AND. (RED_TIME > 2.5*INTERFACE_WIDTH) )EXIT
...
REAL(DP) :: SUMA, SUMB, LAM
 ...
 ELSE
	...
	do i=1,NODE_X
		!IF(iter_cnt==1)then
        !        lam = 2.0*(1.0-RATIO_RHO)*DELTA(I,J,K)*HEAVY(I,J,K)+RATIO_RHO*DELTA(I,J,K)
        !else
                lam = DELTA(I,J,K)
		!endif
       A(i,j,k)= (SGN(i,j,k)*lam)*(GRAD(i,j,k)-1.0_dp)
       B(i,j,k)= GRAD(i,j,k)*DELTA(i,j,k)*lam
    enddo
    enddo
    enddo
    !$omp end parallel do
