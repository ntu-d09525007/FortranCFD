...
do idy = 0, p%glb%num_of_plot-1
    ...
    E_IO = VTK_GEO_XML(nx1=nx1, nx2=nx2, ny1=ny1, ny2=ny2, nz1=nz1, nz2=nz2, NN=nn, &
                       ...
                       Z=reshape(p%loc%zeros(nx1:nx2,ny1:ny2),(/nn/)))
                       
    E_IO = VTK_DAT_XML(var_location = 'node', var_block_action = 'open')
    E_IO = VTK_VAR_XML(NC_NN = nn, varname = 'Phi', var = reshape(p%loc%phi%now(nx1:nx2,ny1:ny2),(/nn/)) )
    E_IO = VTK_VAR_XML(NC_NN = nn, varname = 'Pressure', var = reshape(p%loc%p%now(nx1:nx2,ny1:ny2),(/nn/)) )
    E_IO = VTK_VAR_XML(NC_NN = nn, varname = 'Velocity', varX = reshape(p%loc%nvel%x%now(nx1:nx2,ny1:ny2),(/nn/)),&
                                                        &varY = reshape(p%loc%nvel%y%now(nx1:nx2,ny1:ny2),(/nn/)),&
                                                        &varZ = reshape(p%loc%zeros(nx1:nx2,ny1:ny2),(/nn/)))
