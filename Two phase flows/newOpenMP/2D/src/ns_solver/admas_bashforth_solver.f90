...
do i = p%loc%is, p%loc%ie
    ...
    p%loc%velsrc%x%now(i,j) = p%loc%velsrc%x%now(i,j) + p%loc%velsrc%x%tmp(i,j)
    p%loc%velsrc%y%now(i,j) = p%loc%velsrc%y%now(i,j) + p%loc%velsrc%y%tmp(i,j)
...
do i = p%loc%is, p%loc%ie
    ...
    src = 1.50*p%loc%velsrc%x%now(i,j) - 0.5d0*p%loc%velsrc%x%old(i,j)             
    p%loc%vel%x%now(i,j) = p%loc%vel%x%old(i,j) + p%glb%dt * src
    
    src = 1.50*p%loc%velsrc%y%now(i,j) - 0.5d0*p%loc%velsrc%y%old(i,j)   
    p%loc%vel%y%now(i,j) = p%loc%vel%y%old(i,j) + p%glb%dt * src
